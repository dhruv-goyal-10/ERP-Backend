...
class OTP(models.Model):
    ...
    otp_created_at = models.DateTimeField(default=datetime.datetime(
...
class Teacher(models.Model):
    ...
    sex = models.CharField(
        max_length=10, choices=sex_choice, blank=True, null=True)
    DOB = models.DateField()
    picture = models.ImageField(upload_to='teachers/', height_field=None,
                                width_field=None, max_length=100, blank=True, null=True)
    blood_group = models.CharField(max_length=20, blank=True, null=True)
    pincode = models.IntegerField(blank=True, null=True)
    address = models.CharField(max_length=200, blank=True, null=True)
    city = models.CharField(max_length=200, blank=True, null=True)
    state = models.CharField(max_length=200, blank=True, null=True)
    teacher_phone = models.BigIntegerField(blank=True, null=True, validators=[
                                           MinValueValidator(1000000000), MaxValueValidator(9999999999)])
...
class TeacherFeedback(models.Model):
    ...
    feed = models.IntegerField(default=3, validators=[
...
class AssignClass(models.Model):
    ...
    teacher = models.ForeignKey(
        Teacher, on_delete=models.CASCADE, related_name="assignclass_teacher")
...
class AssignTime(models.Model):
    ...
    class Meta:
        unique_together = (('period', 'day', 'class_id'),
                           ('period', 'day', 'teacher'),)
...
class StudentAttendance(models.Model):
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE)
    student = models.ForeignKey(Student, on_delete=models.CASCADE)
    classattendance = models.ForeignKey(
        ClassAttendance, on_delete=models.CASCADE)
